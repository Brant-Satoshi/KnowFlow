...
| `lib/db/chunks.ts` | chunk 写入、读取、pgvector 相似度检索 |
...
目标是减少页码、重复空白和无意义换行对 embedding 的干扰。
...
2. 识别 Markdown 标题和中文章节标题，写入 `sectionTitle`。
3. 优先在换行、句号、问号、感叹号、分号等边界截断。
...
| chat recall topK | `app/api/chat/stream/route.ts` | `20` | 候选覆盖率 |
| chat maxDistance | `app/api/chat/stream/route.ts` | `0.6` | 过滤弱相关 chunk |
| rerank topN | `app/api/chat/stream/route.ts` | `8` | rerank 后候选窗口 |
| final chunks | `app/api/chat/stream/route.ts` | `5` | prompt 成本和证据噪声 |
